...
void solve() {
    ...
    ll ans = 1;
    for(char x = 'a'; x <= 'z'; x++) { // hidden msg is 1 lettered
        ans = max(ans, pos[x].size() * 1ll * (pos[x].size() - 1) / 2);
        ans = max(ans, (ll)pos[x].size());
    }

    for(char x = 'a'; x <= 'z'; x++) { // 2 lettered
        for(char y = 'a'; y <= 'z'; y++) {
            if (x == y)
                skip;
            ...
            for(int j = 0; j < pos[x].size(); j++) {
                cur += pos[y].size() - (lower_bound(all(pos[y]), pos[x][j]) - pos[y].begin());
            }
