...
ll f(ll x, ll y, ll z) {
    return (x - z) * (x - z) + (x - y) * (x - y) + (z - y) * (z - y);
}

void calc(vector<int> r, vector<int> g, vector<int> b) {
    for(auto v : r) {
        auto y = lower_bound(all(g), v);
        auto x = upper_bound(all(b), v);
        ...
        ans = min(ans, f(v * 1ll, *x * 1ll, *y * 1ll));
//        cout << v << ' ' << *x << ' ' << *y << endl;
    }
}
