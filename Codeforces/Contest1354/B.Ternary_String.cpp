...
void solve() {
    ...
    for(int i = 0; i < (int) p[1].size(); i++) {
        auto n11 = lower_bound(all(p[2]), p[1][i]);
        if (n11 != p[2].end()) {
            int nearest_21 = *n11;
            n11 = lower_bound(all(p[3]), p[1][i]);
            if (n11 != p[3].end()) {
                ans = min(ans, max(*n11, nearest_21) - p[1][i] + 1);
            }
        }
    }

    for(int i = 0; i < (int) p[2].size(); i++) {
        auto n11 = lower_bound(all(p[1]), p[2][i]);
        if (n11 != p[1].end()) {
            int nearest_21 = *n11;
            n11 = lower_bound(all(p[3]), p[2][i]);
//            cout << i << ' ' << nearest_21 << ' ' << *n11 << '\n';
            if (n11 != p[3].end()) {
                ans = min(ans, max(*n11, nearest_21) - p[2][i] + 1);
            }
        }
    }


    for(int i = 0; i < (int) p[3].size(); i++) {
        auto n11 = lower_bound(all(p[1]), p[3][i]);
        if (n11 != p[1].end()) {
            int nearest_21 = *n11;
            n11 = lower_bound(all(p[2]), p[3][i]);
            if (n11 != p[2].end()) {
                ans = min(ans, max(*n11, nearest_21) - p[3][i] + 1);
            }
        }
    }
