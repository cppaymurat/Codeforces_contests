...
void solve() {
    ...
    int lastEnd = n + 1;
    for(int i = m - 1; i >= 1; i--) {
        int new_position = lastEnd - l[i];
        if (minimalistic[i] >= new_position)
            break;
        minimalistic[i] = max(new_position, minimalistic[i]);
        lastEnd = minimalistic[i];
    }
