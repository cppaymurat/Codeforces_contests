...
void solve() {
    ...
    if (n < 4) {
        ...
        vector<int> block(toPush.size(), 0);
        for(int i = 0; i < (int) toPush.size(); i++) {
            for(int j = 1; j < (int) ans.size(); j++) {
                if (toPrint[j - 1] == 0 && norm(abs(toPush[i] - ans[j])) && norm(abs(toPush[i] - ans[j - 1]))) {
                    toPrint[j - 1] = toPush[i];
                    block[i] = 1;
                    break;
                }
            }
