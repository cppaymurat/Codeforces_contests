...
void solve() {
    ...
    sort(all(xy), [](pii a, pii b) {return a.fr + a.sc < b.fr + b.sc;});
    string ans = "";
    int last_x = 0,
        last_y = 0;
    ...
    for(int i = 0; i < xy.size() - 1; i++) {
        ...
        if (xy[i].fr > xy[i + 1].fr || xy[i].sc > xy[i + 1].sc) {
            cout << "NO\n";
            return;
        }

        for(int j = last_x; j < xy[i].fr; j++) {
            ans += "R";
        }

        last_x = xy[i].fr;
        for(int j = last_y; j < xy[i].sc; j++) {
            ans += "U";
        }
        last_y = xy[i].sc;
    }

    for(int i = last_x; i < xy[n - 1].fr; i++) {
        ans += "R";
    }
    for(int i = last_y; i < xy[n - 1].sc; i++) {
        ans += "U";
    }
